...
// K2 Horizon / MoVA system regex: "(?:'[sS]|'[tT]|'[rR][eE]|'[vV][eE]|'[mM]|'[lL][lL]|'[dD])|[^\\r\\n\\p{L}\\p{N}]?(?:\\p{L}|\\p{M}|\\u200C|\\u200D)+|\\p{N}{1,3}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+"
// Same shape as the llama3 pattern above, except the word-run group also admits combining
// marks (\p{M}) and the ZWNJ/ZWJ joiners (‌/‍) so that scripts which rely on them
// (e.g. Devanagari conjuncts) stay attached to their base letter as a single token.
static std::vector<size_t> unicode_regex_split_custom_k2_horizon(const std::string & text, const std::vector<size_t> & offsets) {
    ...
    for (auto offset : offsets) {
        ...
        // regex: \p{L}|\p{M}|‌|‍
        auto _is_word_cpt = [&] (const size_t pos) -> bool {
            const uint32_t cpt = _get_cpt(pos);
        ...
        for (size_t pos = offset_ini; pos < offset_end; /*pos++*/ ) {
            ...
            // regex: (?:'[sS]|'[tT]|'[rR][eE]|'[vV][eE]|'[mM]|'[lL][lL]|'[dD]) // case insensitive
            if (cpt == '\'' && pos+1 < offset_end) {
                uint32_t cpt_next = unicode_tolower(_get_cpt(pos+1));
                if (cpt_next == 's' || cpt_next == 't' || cpt_next == 'm' || cpt_next == 'd') {
                    pos += _add_token(pos+2);
            ...
            // regex: [^\r\n\p{L}\p{N}]?(?:\p{L}|\p{M}|‌|‍)+
            if (!(cpt == '\r' || cpt == '\n' || flags.is_number)) {
                if (_is_word_cpt(pos) || _is_word_cpt(pos+1)) {  // one or more word cpts
                    pos++;
                    while (_is_word_cpt(pos)) {
                        pos++;
                    }
                    _add_token(pos);
                    continue;
